...
## v7.6.1.1 한글 인코딩/검색 긴급 수정

- gaming.tools 응답을 명시적으로 UTF-8 디코딩
- UTF-8을 ISO-8859-1로 잘못 해석한 mojibake 문자열 자동 복구
- 깨진 한글 비율 검증 후 비정상 DB 저장 차단
- 기존 깨진 equipment DB를 백업 후 비우고 재수집
- 브라우저 DB 요청에 cache-busting 적용
- 한글 검색 정규화를 NFC + ko-KR 소문자 기준으로 강화


## v7.6.1.2 운영 안정화

- Pages는 데이터 갱신과 동일한 작업공간의 아티팩트를 배포
- 장비 DB UTF-8 및 한글 품질 검증 강화
- 아이템 페이지 pagination 링크 탐색
- URL 텍스트를 스펙으로 잘못 저장하는 문제 차단
- DD는 섹션별 검증/부분 갱신
- 던전은 루트별 검증/부분 갱신
- 실패한 섹션·루트만 이전 검증본 유지
- 주간 기간을 weekly.json에서 헤더에 강제 동기화
- 개인화된 목표/추천 문구를 범용 저장 기능으로 변경
